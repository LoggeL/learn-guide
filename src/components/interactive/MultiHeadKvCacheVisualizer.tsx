...
const COPY = {
  en: {
    ...
    title: 'Multiple lenses scan the prompt while the KV cache keeps a decode ledger',
    desc: 'Switch the telescope lens, then step through generation. Each head projects the same tokens into a different attention map. The newest query sweeps across cached keys/values instead of rebuilding the whole sky every time.',
    headFocus: 'Lens array',
    decodeStep: 'Decode sweep',
    next: 'Next token',
    reset: 'Reset',
    attentionView: 'Radar sweep for this head',
    cache: 'KV cache ledger',
    cached: 'cached rows',
    newWork: 'fresh row',
    avoided: 'avoided recomputation',
    withoutCache: 'without cache',
    withCache: 'with cache',
    prompt: 'Prompt tokens begin on the shelf. Each new decode step appends one K/V ledger row.',
    reads: 'new query beam reads the cached shelves',
    queries: 'Only the newest token creates a fresh query, key, and value. Earlier K/V rows stay on the ledger and are reused.',
  ...
  de: {
    ...
    title: 'Mehrere Linsen scannen den Prompt, während der KV-Cache ein Decoding-Ledger führt',
    desc: 'Wechsle die Teleskop-Linse und gehe dann durch die Generierung. Jeder Head projiziert dieselben Tokens auf eine andere Attention-Karte. Die neueste Query tastet gecachte Keys/Values ab, statt jedes Mal den ganzen Himmel neu aufzubauen.',
    headFocus: 'Linsen-Array',
    decodeStep: 'Decoding-Sweep',
    next: 'Nächstes Token',
    reset: 'Zurücksetzen',
    attentionView: 'Radar-Sweep für diesen Head',
    cache: 'KV-Cache-Ledger',
    cached: 'gecachete Zeilen',
    newWork: 'frische Zeile',
    avoided: 'vermiedene Neuberechnung',
    withoutCache: 'ohne Cache',
    withCache: 'mit Cache',
    prompt: 'Prompt-Tokens liegen bereits im Regal. Jeder neue Decoding-Schritt fügt eine K/V-Ledger-Zeile an.',
    reads: 'neuer Query-Strahl liest die gecachten Regale',
    queries: 'Nur das neueste Token erzeugt eine frische Query, einen Key und einen Value. Frühere K/V-Zeilen bleiben im Ledger und werden wiederverwendet.',
