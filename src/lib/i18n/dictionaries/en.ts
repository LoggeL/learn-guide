export const en = {
  ...
  batching: {
    ...
    throughputExplain: 'As batch size increases, total throughput rises steeply at first — then plateaus. This happens because LLM inference is memory-bandwidth bound: the GPU must load the entire model weights from DRAM for each forward pass. More batches share this fixed cost, but eventually DRAM bandwidth saturates and adding more requests barely helps.',
    throughputLabel: 'Total Throughput',
    latencyLabel: 'Per-Request Latency',
    batchSizeLabel: 'Batch Size',
    tokPerSec: 'tok/s',
    msPerTok: 'ms/tok',
    plateauExplain: 'The plateau happens because of DRAM bandwidth saturation — not compute saturation. Even at batch size 256, the GPU compute units are still underutilized. The bottleneck is how fast weights can be loaded from memory.',
    improvementNote: '256× more requests, but only ~33× more throughput',
    ...
    prefillDesc: 'All input tokens are processed in parallel in a single forward pass. This phase is compute-bound — the GPU\'s tensor cores are fully utilized. This is where Time to First Token (TTFT) comes from.',
    prefillAnalogy: 'Like a conveyor belt processing all items at once',
    decodeTitle: 'Decode Phase',
    decodeDesc: 'Tokens are generated one at a time, each depending on the previous one. This phase is memory-bound — less than 20% of GPU compute is used. This is where Time Between Output Tokens (TBOT) comes from.',
    ...
    highUtil: 'High GPU utilization',
    lowUtil: 'Low GPU utilization',
    parallelProcess: 'All tokens in parallel',
    sequentialProcess: 'One token at a time',
    ttft: 'TTFT (Time to First Token)',
    tbot: 'TBOT (Time Between Output Tokens)',
    keyInsight: 'In prefill, nearly all latency is compute. In decode, less than 20% is compute — the rest is waiting for memory. Batching helps most during decode because single decode barely uses the GPU.',
    phaseAnimTitle: 'Watch the Phases',
    phaseAnimSubtitle: 'See how a request flows through prefill and decode',
    promptIn: 'Prompt In',
    ...
    lowBatchDesc: 'Low throughput, but each user gets fast responses',
    highBatch: 'High Batch Size',
    highBatchDesc: 'High throughput, but each user waits longer',
    tradeoffInsight: 'The sweet spot depends on your use case: real-time chat needs low latency, while batch processing jobs can maximize throughput.',
    ...
    staticBatchTitle: 'Static Batching (Traditional)',
    staticBatchDesc: 'All requests must start together and the batch waits for the longest request to finish. Shorter requests waste GPU time sitting idle.',
    continuousBatchTitle: 'Continuous Batching (Orca)',
    continuousBatchDesc: 'Finished requests leave immediately and new requests join on the fly. The GPU is never idle as long as there\'s work to do.',
    slotLabel: 'Slot',
    activeLabel: 'Active',
    idleLabel: 'Idle',
    newReqLabel: 'New request joins',
    finishedLabel: 'Finished',

    // Takeaways
    keyTakeaways: 'Key Takeaways',
    takeaway1: 'Batching amortizes the cost of loading model weights across multiple requests, dramatically increasing throughput',
    takeaway2: 'LLM decode is memory-bandwidth bound — even large batch sizes don\'t fully utilize GPU compute',
    takeaway3: 'Prefill is compute-bound (fast, parallel), while decode is memory-bound (slow, sequential) — batching helps most during decode',
    takeaway4: 'Continuous batching eliminates idle GPU time by allowing requests to join and leave the batch dynamically',
  },
}
