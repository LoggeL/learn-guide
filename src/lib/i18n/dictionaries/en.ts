export const en = {
  ...
  localInference: {
    ...
    moeDesc: 'Mixture of Experts (MoE) models are a game-changer for local inference. Instead of activating every parameter for each token, MoE routes each token through only a small subset of "expert" layers — giving you the intelligence of a massive model at a fraction of the cost.',
    moeBenefit1: 'Fraction of Parameters Active',
    moeBenefit1Desc: 'Only a small subset of experts fire per token. An 80B MoE model may only activate 3B parameters — meaning inference speed and memory behave like a 3B model.',
    moeBenefit2: 'Large-Model Intelligence',
    moeBenefit2Desc: 'The full 80B parameters still store knowledge across all experts. You get the reasoning depth of a much larger model, just accessed sparsely.',
    moeBenefit3: 'Perfect for Limited VRAM',
    moeBenefit3Desc: 'While you still need to load all parameters, quantized MoE models fit in surprisingly little VRAM — and inference is fast because only active params need compute.',
    ...
    moeInsight: 'Qwen MoE activates only 3B of its 80B parameters per token — giving you 70B-class intelligence at 3B-class speed, fitting comfortably in 12 GB VRAM.',
